  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/[0.06] bg-ink/70 backdrop-blur-xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        <a href="#home" {...openInNewTab} className="focus-ring flex shrink-0 items-center rounded-md">
          <Image
            src="/tdz-logo.png"
            alt="Tubagus Dhaifullah Zuhdi"
            width={362}
            height={306}
          ...
          <div className="mx-auto flex max-w-7xl flex-col gap-2">
            {links.map((link) => (
              <a
                ...
                className="rounded-md px-3 py-3 text-sm text-neutral-300"
              >
                {link.label}
              </a>
            ))}
